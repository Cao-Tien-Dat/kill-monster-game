        ...
        if (meteoroid[i].visible == WHITE) {
            if (meteoroid[i].action_image == 1) {
                view_render.drawBitmap(meteoroid[i].x,
                                       meteoroid[i].y,
                                       bitmap_meteoroid_I,
                                       SIZE_BITMAP_METEOROIDS_X_1,
                                       SIZE_BITMAP_METEOROIDS_Y_1,
                                       WHITE);
            } else if (meteoroid[i].action_image == 2) {
                view_render.drawBitmap(meteoroid[i].x,
                                       meteoroid[i].y,
                                       boom,
                                       SIZE_BITMAP_METEOROIDS_X_3,
                                       SIZE_BITMAP_METEOROIDS_Y_3,
                                       WHITE);
            } else if (meteoroid[i].action_image == 3) {
                view_render.drawBitmap(meteoroid[i].x,
                                       meteoroid[i].y,
                                       bitmap_meteoroid_III,
                                       SIZE_BITMAP_METEOROIDS_X_2,
                                       SIZE_BITMAP_METEOROIDS_Y_2,
                                       WHITE);
            } else if (meteoroid[i].action_image == 4) {
                view_render.drawBitmap(meteoroid[i].x,
                                       meteoroid[i].y,
                                       bitmap_meteoroid_IV,
                                       SIZE_BITMAP_METEOROIDS_X_2,
                                       SIZE_BITMAP_METEOROIDS_Y_2,
                                       WHITE);
            } else if (meteoroid[i].action_image == 5) {
                view_render.drawBitmap(meteoroid[i].x,
                                       meteoroid[i].y,
                                       bitmap_meteoroid_V,
                                       SIZE_BITMAP_METEOROIDS_X_5,
                                       SIZE_BITMAP_METEOROIDS_Y_5,
                                       WHITE);
            } else if (meteoroid[i].action_image == 6) {
                if (ar_game_score > 150) {
                    view_render.drawBitmap(meteoroid[i].x,
                                           meteoroid[i].y,
                                           bitmap_meteoroid_VI,
                                           SIZE_BITMAP_METEOROIDS_X_6,
                                           SIZE_BITMAP_METEOROIDS_Y_6,
                                           WHITE);
                }
            } else if (meteoroid[i].action_image == 7) {
                view_render.drawBitmap(meteoroid[i].x,
                                       meteoroid[i].y,
                                       gift_sp,
                                       20,
                                       10,
                                       WHITE);
            } else if (meteoroid[i].action_image == 8){
				view_render.drawBitmap( meteoroid[i].x,
										meteoroid[i].y,
										gift_sp1,
										20,
										10,
										WHITE);
			} else if (meteoroid[i].action_image == 9){
				view_render.drawBitmap( meteoroid[i].x,
										meteoroid[i].y,
										bossphase_1,
										60,
										60,
										WHITE);
			} else if (meteoroid[i].action_image == 10){
				view_render.drawBitmap( meteoroid[i].x,
										meteoroid[i].y,
										bossphase_2,
										60,
...
void scr_archery_game_handle(ak_msg_t* msg) {
	...
	case AR_GAME_TIME_TICK: {
		...
		task_post_pure_msg(AR_GAME_METEOROID_ID,	AR_GAME_CHECK_METEOROID_LEVEL_3);
	}
		break;
